...
export default function Login(){
 const [err,setErr]=useState(""),[busy,setBusy]=useState(false),router=useRouter(),handled=useRef(false);

 async function routeUser(user:Parameters<typeof bootstrapAuthenticatedUser>[0]){
   const target=await bootstrapAuthenticatedUser(user);
 ...
 async function google(){
   setBusy(true);setErr("");
   try{
     const result=await beginGoogleAuth("login");
     if(result)await routeUser(result.user);
   }catch(x){
     setErr(isTransientFirestoreError(x)?friendlyFirestoreError(x):friendlyAuthError(x));
     setBusy(false);
   }
 ...
 async function submit(e:React.FormEvent<HTMLFormElement>){
   ...
   try{
     ...
     const c=await signInWithEmailAndPassword(auth,String(f.get("email")).trim(),String(f.get("password")));
     await routeUser(c.user);
   }catch(x){
     setErr(isTransientFirestoreError(x)?friendlyFirestoreError(x):friendlyAuthError(x));
 ...
 <div className="eyebrow">Secure member access</div><h1>Welcome back.</h1><p className="sub">Sign in to continue to your member workspace, collections, earnings and referral tools.</p>
 ...
 <div className="authLinks"><Link href="/forgot-password">Forgot password?</Link><Link href="/join">Create account</Link></div>
 {err&&<div className="error">{err}</div>}<button className="btn wide" disabled={busy}>{busy?"Signing in…":"Sign in →"}</button>
 </form></div></main>
}